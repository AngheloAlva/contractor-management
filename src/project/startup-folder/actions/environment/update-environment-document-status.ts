...
export async function undoEnvironmentDocumentReview({
	userId,
	documentId,
}: UndoDocumentReviewParams) {
	try {
		const document = await prisma.environmentDocument.update({
			where: {
				id: documentId,
			},
			data: {
		...
		const folder = await prisma.environmentFolder.update({
			where: {
		...
		logActivity({
			...
			entityType: "EnvironmentDocument",
			metadata: {
